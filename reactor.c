...
_get_event_t(reactor_t *r) {
    r->iter ++;
    // 确保 r->iter 始终保持在 [0, MAX_CONN] 这个有效范围内
...
int
set_nonblock(int fd) {
    // 获取当前文件描述符 fd 的文件状态标志
    // 这些标志可以是 O_RDONLY、O_WRONLY、O_RDWR、O_NONBLOCK 等等
...
// 将长度为 sz 的 buf 写入到 e 关联的 fd 的内核态发送缓冲区
int
event_buffer_write(event_t *e, void * buf, int sz) {
    buffer_t *out = evbuf_out(e);
    // 用户态发送缓冲区 out 为空，说明内核的发送缓冲区可能有空间
    // 尝试将数据 buf 直接写入内核的发送缓冲区
    if (buffer_len(out) == 0) {
    ...
    // 用户态发送缓冲区 out 不为空，说明内核的发送缓冲区已经没有空间
    // 将数据 buf 追加到新的用户态发送缓冲区 out 中
